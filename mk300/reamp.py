...
silence). The field is restored afterwards. Needs the `reamp` extra: pip install "mk300[reamp]".
"""
...
BLOCK = 2048
SILENCE_DBFS = -60.0
USB_AUDIO_FIELD = 0x3D
USB_AUDIO_RESAMPLE = 2
...
def reamp(di, out, *, tail_s: float = 2.0, mono: bool = False, device=None,
          device_name: str = DEVICE, playrec=None) -> ReampResult:
    """`device` is an mk300.device.MK300 (sets USB Audio = RESAMPLE and restores it); None = leave the
    global alone. `playrec(play, device_name) -> recorded` is injectable for tests."""
    ...
    if device is not None:
        previous = device.read_global()[USB_AUDIO_FIELD]
        device.set_u8(USB_AUDIO_FIELD, USB_AUDIO_RESAMPLE, p.SPACE_STATUS)
    try:
        rec = playrec(play, device_name)[:total]
    finally:
        if device is not None and previous is not None:
            device.set_u8(USB_AUDIO_FIELD, previous, p.SPACE_STATUS)
