"""Regenerate docs/catalog.md and skills/mvave/reference.md from the MK-300 profile catalog + the CLI."""
...
from mvave.devices import mk300 as pr  # noqa: E402

cat = json.loads((ROOT / "mvave" / "devices" / "mk300_catalog.json").read_text())
...
lines = ["# MK-300 model catalog", "",
         "Generated by `tools/build_reference.py` from `mvave/devices/mk300_catalog.json` (M-EFCS 3.7.1413 dropdowns, "
...
help_txt = subprocess.run([sys.executable, "-m", "mvave"], capture_output=True, text=True, cwd=ROOT, env={"PYTHONPATH": str(ROOT), "PATH": "/usr/bin:/bin"}).stdout
ref = ["# mvave reference (MK-300 profile)", "", "## Commands", "", "```", help_txt.strip(), "```", "",
